...
void dfs(int bit,int d) {
    ...
    if (d<3) cout << bit << " " << d << endl;
    for(int i=0;i<used_wires[bit].size();i++) {
        int gate1 = wire_gate[used_wires[bit][i]];
        for(int k=bit+1;k<50;k++) {
            for(int j=0;j<used_wires[k].size();j++) {
                if (find(used_wires[bit].begin(),used_wires[bit].end(),used_wires[k][j])!=used_wires[bit].end()) continue;
                int gate2 = wire_gate[used_wires[k][j]];
                string out1 = gates[gate1].out;
                string out2 = gates[gate2].out;
                if (swapped[out1] || swapped[out2]) continue;
                if (out1[0]!='z' && out2[0]!='z') continue;
                if (out1 != out2) {
                    gates[gate1].out = out2;
                    gates[gate2].out = out1;
                    swapped[out1] = true;
                    swapped[out2] = true;
                    bool ok = true;
                    for(int x=0;x<=bit;x++)
...
int main() {
    ...
    for(int i=0;i<50;i++) {
        cout << i << ": ";
        for(int k=0;k<used_wires[i].size();k++) {
            cout << used_wires[i][k] << " ";
        }
        cout << endl;
    }
